...
如果配置 DeepSeek API Key，系统会只对筛选后的 Top 20 新闻调用模型，用于改进中文标题、核心总结、中文大意和“为什么重要”。原始标题、来源等级、发布时间、规则分数和原文链接仍会保留。
...
本项目每天只处理 Top 20 条新闻，预计每天成本约 `$0.01 - $0.03`，每月约 `$0.3 - $1`，折合人民币大约 `2 - 8 元`。实际费用取决于新闻摘要长度、输出长度、缓存命中和服务商最新价格。
...
当前已经支持可选 LLM 增强。后续如果要升级为更深度的 API 总结模式，建议仍然只对规则筛选后的 Top 10 或 Top 20 新闻做总结，并继续保留：
